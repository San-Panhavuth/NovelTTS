...
def _default_output_dir() -> Path:
    return Path(__file__).parent / "fixtures" / "attribution" / "gold" / "dev"


def _is_gold_test_dir(path: Path) -> bool:
    normalized = [part.lower() for part in path.resolve().parts]
    return len(normalized) >= 3 and normalized[-3:] == ["gold", "test"]
...
async def _export_fixtures(
    ...
    async with SessionLocal() as session:
        ...
        characters = (await session.execute(select(Character).where(Character.book_id == book.id))).scalars().all()
        ...
        for chapter_idx, genre in chapter_specs:
            ...
            if not segments:
                raise ValueError(
                    f"Chapter {chapter_idx} has no segments. Process and review it first before exporting fixtures."
                )
            ...
            expected = [
                {
                    ...
                    "character": character_name_by_id.get(segment.character_id) if segment.character_id else None,
                }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Export attribution fixtures from existing book chapter text + reviewed segments"
    )
    ...
    parser.add_argument(
        "--allow-locked-test-write",
        action="store_true",
        help="Allow writing directly into gold/test fixtures (normally blocked)",
    )
    args = parser.parse_args()

    if _is_gold_test_dir(args.out_dir) and not args.allow_locked_test_write:
        raise ValueError(
            "Refusing to write directly into gold/test fixtures. "
            "Write to gold/dev first, then copy reviewed files into gold/test intentionally. "
            "Use --allow-locked-test-write only for explicit lock updates."
        )
